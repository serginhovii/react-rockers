...
import { useRocksData } from "../../hooks/useRocksData";
...
const Rocks = () => {
    ...
    const { rocks, loading} = useRocksData({
    ...
    const submit = (event) => {
        ...
        if (address) {
            ...
            setSubmitted(true)
            if (isValid) push(`/rocks?address=${address}`)
        } else {
            push("/rocks")
        }
    ...
    return (
        ...
        <form onSubmit={submit}>
            <FormControl>
                ...
                {
                    submitted && !validAddress && <FormHelperText>Invalid address</FormHelperText>
                }
            </FormControl>
        </form>
        {loading ? <Loading /> :
        <Grid templateColumns="repeat(auto-fill, minmax(250px, 1fr))" gap={6}>
            {
                rocks.map(({name, image, tokenId}) => 
                    <Link key={tokenId} to={`/rocks/${tokenId}`}>
                    <RockCard image={image} name={name.replace("Rocks", "Rocker")} />
                    </Link>
                )
            }
        </Grid>
